...
public class SQLConnection implements Connection
{
  ...
  private List<DataBaseTable> _existingTemporaryTables = new ArrayList<DataBaseTable> ();
  ...
  public void dropTemporaryTable (DataBaseTable table) throws ReturnStatusException {
    DataBaseTable existingTable = containsTemporaryTable (table.getTableName ());
    if (table != null) {
      table.dropTable ();
      _existingTemporaryTables.remove (existingTable);
    }
  }
  ...
  public void close () throws SQLException {
    try {
      // before we close the connection we need to make sure we are dropping all
      // temporary tables, if they have not already been dropped.
      dropTemporaryTables ();
  ...
  private void dropTemporaryTables () throws ReturnStatusException {
    for (DataBaseTable table : _existingTemporaryTables) {
      table.dropTable ();
    }
    _existingTemporaryTables.clear ();
  }

  private DataBaseTable containsTemporaryTable (String name) {
    // we will not distinguish between SQL server table variables and
    // temporary tables as defined in :
    // http://www.sqlteam.com/article/temporary-tables
    for (DataBaseTable table : _existingTemporaryTables) {
      if (table.matchesName (name))
        return table;
    }
    return null;
  }
